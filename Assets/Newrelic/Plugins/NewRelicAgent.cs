//
//  New Relic for Mobile -- Unity edition
...
public class NewRelicAgent : MonoBehaviour
{
	...
	internal void StartAgent (string message)
	{
		...
		UnityEngine.Debug.Log ("New Relic agent starting with app token " + appToken);
		...
		try {
			if (validatePluginImpl ()) {
				...
				UnityEngine.Debug.Log ("New Relic agent started default interaction " + id);
				runDefaultTrace ();
				stopDefaultInteraction ();
			}
		} catch (Exception e) {
	...
	static public void NoticeNetworkRequest (string URL,
	                                         string httpMethod,
	                                         Timer timer,
	...
	{
		if (validatePluginImpl ()) {
			instance.agentInstance.noticeNetworkRequest (URL, httpMethod, timer, headers, httpStatusCode, bytesSent, bytesReceived, responseData, parameters);
		}
	}
	...
	/// </summary>
	/// <returns><c>true</c>, if event was recorded, <c>false</c> otherwise.</returns>
	/// <param name="name">Name of the event. Will be stored in the 'category' attribute of Mobile events in New Relic Insights</param>
	...
	static public bool RecordEvent (string name,
	                                Dictionary<string,object> attributes)
	{
		bool result = false;

		if (validatePluginImpl ()) {
			instance.agentInstance.recordEvent (name, attributes);
		}
